...
export default function AgentStatusPage() {
  ...
  const profitAgents = agentUpdate?.topProfitAgents ?? [...snapshots].sort((a, b) => b.returnRate - a.returnRate).slice(0, 8);
  const lossAgents = agentUpdate?.topLossAgents ?? [...snapshots].sort((a, b) => a.returnRate - b.returnRate).slice(0, 8);

  return (
    <main className="min-h-[calc(100dvh-56px)] p-3 sm:p-4" style={{ backgroundColor: TERMINAL.page }}>
      <div className="mx-auto max-w-[1800px] space-y-3">
        <AgentOverviewPanel agentUpdate={agentUpdate} stock={marketState?.stock ?? null} />

        <section className="grid grid-cols-1 2xl:grid-cols-[280px_minmax(0,1fr)_360px] gap-3">
          <aside className="space-y-3 min-w-0">
            <AgentTypeListPanel groups={groups} selectedType={selectedType} onSelect={setSelectedType} />
          ...
          <div className="space-y-3 min-w-0">
            ...
            <AgentGroupCards groups={groups} agents={snapshots} behaviorEvents={agentUpdate?.behaviorEvents ?? []} />
          ...
          <aside className="space-y-3 min-w-0">
            ...
            <AgentBehaviorTimeline events={agentUpdate?.behaviorEvents ?? []} />
          </aside>
        </section>
